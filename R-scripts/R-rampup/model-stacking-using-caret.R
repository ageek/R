#Model Stacking using Caret package
...
library(RCurl)
urlData <- getURL('https://raw.githubusercontent.com/hadley/fueleconomy/master/data-raw/vehicles.csv', ssl.verifypeer = FALSE)
...
model_rpart <- train(ensembleData[,predictors], 
                     as.factor(ensembleData[,labelName]), 
...
final_blender_model <- train(blenderData[,predictors], 
                             blenderData[,labelName], method='gbm', 
                             trControl=myControl)
...
##############with prob only
predictors <- names(blenderData)[c(25,26,27)]
final_blender_model <- train(blenderData[,predictors], 
                             as.factor(blenderData[,labelName]), method='gbm', 
                             trControl=myControl)
